...
export default function Home() {
  ...
  const onFinish = async ({ link }: FormValues) => {
    try {
      ...
      setStatus('error');
      setMessage(error.response?.data?.error_description || 're enter link.');
    }
    }
    ...
    return( 
      <Layout>
        <Head>
          <title> Bridge City Tech URL Shrinker </title>
        ...
        <Content className={styles.content}>
          <div className = {styles.shortner}>
            <Title level={5}> Copy &amp; Paste your link to shorten</Title>
            <Form
            form={form}
            onFinish={onFinish}
